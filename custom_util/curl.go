...
func HttpProxyFileUpload(file *multipart.FileHeader, fileKey string, addFields map[string]string,
	...
	if err != nil {
		clog.Error("Upload Create form file failed", zap.Error(err))
		return
	}
	...
	if err != nil {
		clog.Error("Write to form file failed", zap.Error(err))
		return
	}
	for fieldKey, fieldVal := range addFields {
		if err = writer.WriteField(fieldKey, fieldVal); err != nil {
			clog.Error("WriteField failed", zap.Error(err))
			return
		}
	}
	// 发送表单
	...
	if err != nil {
		clog.Error("Post failed", zap.Error(err))
