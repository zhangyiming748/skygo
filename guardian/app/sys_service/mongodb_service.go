...
func (t *MgoOrmSession) GetQuery() *orm.Query {
	return t.Query
}

func NewMgoSession(table string) *MgoOrmSession {
	return &MgoOrmSession{*orm.NewMgoSession(GetDefaultMongodbDatabase().C(table), orm.UrlParams{})}
}

func NewMgoSessionWithCond(table string, params qmap.QM) *MgoOrmSession {
	newMgoSession := MgoOrmSession{*orm.NewMgoSession(GetDefaultMongodbDatabase().C(table), orm.UrlParams{})}
...
func (this *MgoOrmSession) SetTransformer(trans transformer.Transformer) *MgoOrmSession {
	this.MongoSession.SetTransformer(this.getOrmTransformer(trans))
	return this
}

func (this *MgoOrmSession) getOrmTransformer(t transformer.Transformer) *orm.Transformer {
