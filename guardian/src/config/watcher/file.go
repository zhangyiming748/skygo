...
func (w *watcher) Handle(event Event) {
	...
	case w.C <- event:
	default:
		log.Printf("event channel full discard file %s update event", event.Key)
	}
}
...
func (f *file) watchproc(base string) {
	if err := f.notify.Add(base); err != nil {
		log.Printf("create fsnotify for base path %s fail %s, reload function will lose efficacy", base, err)
		return
	}
	log.Printf("start watch config: %s", base)
	for event := range f.notify.Events {
		...
		case event.Op&fsnotify.Write == fsnotify.Write, event.Op&fsnotify.Create == fsnotify.Create:
			if err := f.reloadFile(event.Name); err != nil {
				log.Printf("load file: %s error: %s, skipped", event.Name, err)
			}
		default:
...
func (f *file) reloadFile(fpath string) (err error) {
	...
	f.wmu.RUnlock()
	log.Printf("reload config: %s events: %d\n", key, n)
	return
}
